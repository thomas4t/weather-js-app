...
variable "name" {
  type        = string
  description = "Elastic Beanstalk environemnt name"
}
...
variable "loadbalancer_access_logs_enabled" {
  type = bool
  default = false
  description = "Loadbalancer access log enabled"
}
...
variable "loadbalancer_connection_draining_enabled" {
  type        = bool
  default     = false
  description = "Connection draining for maximum timeout to drain connection"
}
...
variable "deployment_policy" {
  type        = string
  default     = null
  description = "Deployment policy ... default AllAtOnce"
}
...
variable "vpc_subnets" {
  type        = list(string)
  default     = null
  description = "Specify subnets for Instances to be deployed to (private subnet recommended)"
}
variable "vpc_elb_subnets" {
  type = list(string)
  default = null
  description = "Specify subnets for load balancer deployed to"
}
...
variable "iam_role_name" {
  type = string
  description = "EC2 IAM ROle for EB attached to."
  default = null
}

variable "iam_role_instance_profile_name" {
  type = string
  description = "Name of EC2 IAM Role instance profile"
  default = null
}
...
variable "iam_role_service_name" {
  type = string
  default = null
  description = "EC2 IAM Service role, when missing, new role is created"
}

variable "aws_s3_bucket_elb_logs" {
  type = string
...
variable "proxy_server" {
  type = string
  default = "none"
  description = "The proxy server for EB (https://docs.aws.amazon.com/elasticbeanstalk/latest/dg/command-options-specific.html)"
}